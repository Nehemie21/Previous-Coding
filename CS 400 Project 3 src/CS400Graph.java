...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
        	//sets all the values for this Path.
        	this.start = copyPath.start;
        	this.distance = copyPath.distance+extendBy.weight;
        	//creates a new linkedlist of the object and extends it, 
        	//so the old one stays the same.
    ...
    protected Path dijkstrasShortestPath(T start, T end) {   
    	...
    	Hashtable<T, Vertex> visitedVerticies = new Hashtable<>();
    	...
    	while(!queue.isEmpty()) {
    		...
    		//found).
    		if(!visitedVerticies.contains(path.end.data)) {
    			visitedVerticies.put(path.end.data, path.end);
    			//checks if this is the correct end for the path we are searching for
    			if(path.end.data == end) {
    				return path;
    			}
    			//updates the queue if not
    			else {
    				for(Edge e: path.end.edgesLeaving) {
    					queue.add(new Path(path,e));
    				}
    			}
    		}
    	}
    	throw new NoSuchElementException();
    	//make sure none of them have already been visited, and if they have update shortest path to them
